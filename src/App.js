...
const App = () => {
  ...
  const handleAddSchedule = () => {
    ...
    setScheduleModal({ ...scheduleModal, isOpen: false, label: '', time: '' });
  };
      ...
      if (valid.length >= 2) {
        ...
        while (prevM > 1) {
          ...
          for (let i = 0; i < curM; i++) {
            r.push({ id: `${rN}-${i}`, t1: rounds[rN-1][i*2].winner, t2: rounds[rN-1][i*2+1].winner, isBye: false, winner: null, scores: [{t1:'',t2:''}] });
          }
          rounds.push(r);
          prevM = curM; rN++;
        }
        newBrackets[cat] = rounds;
      }
    });
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
      <div className="bg-white p-8 rounded-3xl shadow-2xl w-full max-w-sm">
        ...
        <form onSubmit={handleLogin} className="space-y-3">
          ...
          <input type="text" value={loginData.username} onChange={e => setLoginData({...loginData, username: e.target.value})} className="w-full px-4 py-2.5 bg-slate-50 border rounded-xl font-bold text-xs outline-none" placeholder="Kullanıcı" />
          <input type="password" value={loginData.password} onChange={e => setLoginData({...loginData, password: e.target.value})} className="w-full px-4 py-2.5 bg-slate-50 border rounded-xl font-bold text-xs outline-none" placeholder="Şifre" />
          <button type="submit" className="w-full bg-emerald-600 text-white font-black py-3 rounded-xl shadow-md uppercase text-[10px] tracking-widest">GİRİŞ YAP</button>
        </form>
    ...
    <div className="min-h-screen bg-[#f1f5f9] flex flex-col font-sans text-slate-900">
      <header className="bg-[#064e3b] text-white p-3 shadow-xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
             ...
             {userRole === 'admin' && (
               <button onClick={() => setResetModal({ isOpen: true, password: '', error: '' })} className="p-1.5 bg-red-600/20 rounded-lg hover:bg-red-600 transition-all"><RotateCcw size={14}/></button>
             )}
             <button onClick={handleLogout} className="p-1.5 bg-white/5 rounded-lg hover:bg-white/10"><LogOut size={14}/></button>
          </div>
        </div>
      </header>
      ...
      <main className="flex-grow p-3 sm:p-4 max-w-7xl mx-auto w-full flex flex-col overflow-hidden">
        <div className="flex gap-1.5 mb-4 overflow-x-auto no-scrollbar pb-1">
          {CATEGORIES.map(cat => (
            <button key={cat} onClick={() => setActiveTab(cat)} className={`px-4 py-2 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all ${activeTab === cat ? 'bg-[#064e3b] text-white shadow-md' : 'bg-white text-slate-400 shadow-sm'}`}>{cat}</button>
          ))}
        </div>
          ...
          <div className="flex flex-col gap-4 animate-in fade-in">
             ...
             <div className="flex items-center gap-2 overflow-x-auto no-scrollbar py-1">
                ...
                {['Tümü', ...MATCH_CATEGORIES, 'Genel'].map(f => (
                  <button key={f} onClick={() => setScheduleFilter(f)} className={`px-3 py-1 rounded-full text-[8px] font-black uppercase transition-all border ${scheduleFilter === f ? 'bg-emerald-600 border-emerald-600 text-white' : 'bg-white border-slate-200 text-slate-500'}`}>{f}</button>
                ))}
             </div>
             ...
             <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-xl border relative">
                <div className="flex justify-between items-center mb-6">
                  ...
                  {userRole === 'admin' && (
                    <button onClick={() => setScheduleModal({ ...scheduleModal, isOpen: true })} className="px-3 py-1.5 bg-emerald-600 text-white rounded-lg text-[9px] font-black flex items-center gap-1.5"><Plus size={12}/> PROGRAMA EKLE</button>
                  )}
                </div>
            ...
            <div className="bg-white rounded-2xl p-6 border shadow-xl max-w-3xl mx-auto w-full animate-in fade-in">
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex gap-2">
                  <button onClick={fillRandom} className="px-3 py-1.5 text-[8px] font-black bg-slate-100 rounded-lg flex items-center gap-1 uppercase"><Shuffle size={12}/> Rastgele</button>
                  <button onClick={addTeam} className="px-3 py-1.5 text-[8px] font-black bg-emerald-50 text-emerald-700 rounded-lg flex items-center gap-1 uppercase"><Plus size={12}/> Ekle</button>
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-6 max-h-[300px] overflow-y-auto pr-2">
                {(teams[activeTab] || []).map((team, index) => (
                  <div key={index} className="group bg-slate-50 border p-2 rounded-lg flex items-center gap-2 transition-all hover:border-emerald-200">
                    ...
                    <button onClick={() => removeTeam(index)} className="p-1 text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100"><Trash2 size={12}/></button>
                  </div>
                ))}
              </div>
              <button onClick={startTournament} className="w-full bg-[#064e3b] text-white font-black py-3.5 rounded-xl shadow-lg flex items-center justify-center gap-2 text-xs uppercase tracking-widest italic active:scale-95 transition-all"><Play size={16} fill="currentColor"/> Fikstürü Canlıya Al</button>
            </div>
          ) : (
            <div className="flex-grow overflow-x-auto bg-white border rounded-2xl p-4 shadow-inner animate-in fade-in">
              {brackets?.[activeTab] ? (
                <div className="flex gap-8 min-w-max h-full items-center px-2">
                  {brackets[activeTab].map((round, rIdx) => (
                    <div key={rIdx} className="flex flex-col gap-6 w-44">
                      <h4 className="text-center text-[7px] font-black text-slate-400 uppercase tracking-widest">{rIdx === brackets[activeTab].length - 1 ? 'FİNAL' : `TUR ${rIdx + 1}`}</h4>
                            ...
                            <div className="p-2 space-y-1.5">
                              {[ {n: match.t1, s: match.scores?.[0]?.t1 || '0', win: match.winner === match.t1}, {n: match.t2, s: match.scores?.[0]?.t2 || '0', win: match.winner === match.t2} ].map((side, i) => (
                                <div key={i} className={`flex justify-between items-center p-1.5 rounded-lg text-[9px] font-black transition-all ${side.win ? 'bg-[#064e3b] text-white' : 'bg-slate-50 text-slate-600'}`}>
                                  <span className="truncate max-w-[90px]">{side.win && <Check size={10} className="inline mr-1"/>}{side.n || '...'}</span>
                ...
                <div className="flex flex-col items-center justify-center h-full min-h-[300px] text-slate-300">
                  ...
                  <p className="font-black text-[9px] uppercase tracking-widest text-center">Yönetici kurulumu tamamladığında<br/>canlı sonuçlar akacaktır.</p>
                </div>
              )}
            </div>
          )
             ...
             <div className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <input type="time" value={scheduleModal.time} onChange={e => setScheduleModal({...scheduleModal, time: e.target.value})} className="w-full p-2 bg-slate-50 border rounded-lg text-xs font-bold outline-none" />
                  </div>
                ...
                <div className="space-y-1">
                  ...
                  <input type="text" value={scheduleModal.label} onChange={e => setScheduleModal({...scheduleModal, label: e.target.value})} className="w-full p-2 bg-slate-50 border rounded-lg text-xs font-bold outline-none" placeholder={scheduleModal.type === 'match' ? "Örn: Takım A vs Takım B" : "Örn: Öğle Yemeği"} />
                </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-sm overflow-hidden animate-in zoom-in duration-200 shadow-2xl">
            ...
            <div className="p-6 space-y-6 text-center">
               ...
               <div className="space-y-2 pt-2 border-t px-2">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => setScoreModal({ ...scoreModal, editMode: !scoreModal.editMode })} className="flex-1 bg-slate-100 text-slate-600 font-bold py-2 rounded-lg text-[8px] uppercase flex items-center justify-center gap-1 hover:bg-slate-200">
