...
const App = () => {
  ...
  const [rules, setRules] = useState("Turnuva kuralları henüz belirlenmedi."); // Yeni state
  ...
  const handleSaveRules = () => {
    syncToCloud({ rules });
    alert("Kurallar başarıyla kaydedildi ve yayınlandı.");
  };
  ...
  const handleGlobalReset = () => {
    if (resetModal.password === '1234') {
      ...
      setTeams(emptyTeams);
      setSchedule([]);
      setRules("Turnuva kuralları henüz belirlenmedi.");
      syncToCloud({ brackets: emptyState, champions: emptyState, teams: emptyTeams, schedule: [], rules: "Turnuva kuralları henüz belirlenmedi." });
    ...
    <div className="min-h-screen bg-[#f1f5f9] flex flex-col font-sans text-slate-900">
      ...
      <main className="flex-grow p-3 sm:p-4 max-w-7xl mx-auto w-full flex flex-col overflow-hidden">
        <div className="flex gap-1.5 mb-4 overflow-x-auto no-scrollbar pb-1">
          {CATEGORIES.map(cat => (
            <button key={cat} onClick={() => setActiveTab(cat)} className={`px-4 py-2 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all whitespace-nowrap ${activeTab === cat ? 'bg-[#064e3b] text-white shadow-md' : 'bg-white text-slate-400 hover:text-slate-600'}`}>{cat}</button>
          ))}
        </div>
        ...
        {activeTab === 'Kurallar' ? (
          <div className="bg-white rounded-2xl p-6 border shadow-xl max-w-4xl mx-auto w-full animate-in fade-in">
             <div className="flex items-center gap-2 mb-6">
                ...
                <h2 className="text-lg font-black uppercase italic">Turnuva Kuralları</h2>
             </div>
             {userRole === 'admin' ? (
               <div className="space-y-4">
                 <textarea 
                   ...
                   placeholder="Turnuva kurallarını buraya yazın veya yapıştırın..."
                   className="w-full h-64 p-4 bg-slate-50 border rounded-xl font-medium text-sm text-slate-700 outline-none focus:ring-2 focus:ring-emerald-500 resize-y"
                 />
                 <button onClick={handleSaveRules} className="bg-emerald-600 text-white px-6 py-3 rounded-xl font-black text-xs uppercase tracking-widest shadow-md hover:bg-emerald-700 active:scale-95 transition-all flex items-center gap-2"><Check size={16}/> Kuralları Yayınla</button>
               </div>
             ) : (
               <div className="bg-slate-50 p-6 rounded-xl border">
                 <p className="whitespace-pre-wrap font-medium text-sm text-slate-700 leading-relaxed">{rules}</p>
               </div>
             )}
          </div>
        ) : activeTab === 'Program' ? (
                      ...
                      {round.map((match, mIdx) => (
                        <div key={mIdx} className="relative group">
                          {userRole === 'admin' && !match.isBye && (
                             <button 
                               onClick={() => setScoreModal({ isOpen: true, category: activeTab, rIdx, mIdx, editMode: true })}
                               className="absolute -top-2 -right-2 z-10 p-1 bg-slate-800 text-white rounded-full shadow-md opacity-0 group-hover:opacity-100 transition-all hover:bg-slate-700" title="Eşleşmeyi Düzenle">
                               <Edit3 size={10}/>
                             </button>
                          )}
                          <div className={`bg-white border-2 rounded-xl shadow-sm transition-all ${userRole === 'admin' && !match.isBye ? 'cursor-pointer hover:border-emerald-500' : 'border-slate-50'}`}
                            ...
                            <div className="p-2 space-y-1.5">
                              {[ {n: match.t1, s: match.scores?.[0]?.t1 || '0', win: match.winner === match.t1}, {n: match.t2, s: match.scores?.[0]?.t2 || '0', win: match.winner === match.t2} ].map((side, i) => (
                                <div key={i} className={`flex justify-between items-center p-1.5 rounded-lg text-[9px] font-black transition-all ${side.win ? 'bg-[#064e3b] text-white' : 'bg-slate-50 text-slate-600'}`}>
                                  <span className="truncate max-w-[90px] flex items-center gap-1.5">{side.win && <Check size={10} className="inline mr-1"/>}{side.n || '...'}</span>
          ...
          <div className="bg-white rounded-2xl w-full max-w-sm overflow-hidden animate-in zoom-in duration-200 shadow-2xl">
            ...
            <div className="p-6 space-y-6 text-center">
               ...
               <div className="space-y-2 pt-2 border-t px-2">
                  ...
                  {scoreModal.editMode ? (
                    <button onClick={() => setScoreModal({ isOpen: false, category: null, rIdx: null, mIdx: null, editMode: false })} className="w-full bg-slate-100 text-slate-600 font-bold py-2 rounded-lg text-[8px] uppercase flex items-center justify-center gap-1 hover:bg-slate-200 transition-colors">
                      <X size={12}/> İPTAL ET
                    </button>
                  ) : (
                    <button onClick={clearMatch} className="w-full bg-red-50 text-red-600 font-bold py-2 rounded-lg text-[8px] uppercase flex items-center justify-center gap-1 hover:bg-red-100 transition-colors">
